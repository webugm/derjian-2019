...
function op_insert($kind,$sn=""){
  ...
  $_POST['target'] = db_filter($_POST['target'], '');
  ...
  if($sn){
    ...
    (`title`, `enable`, `sort`, `kind`, `url`, `target`, `content`)
    VALUES 
    ( '{$_POST['title']}', '{$_POST['enable']}', '{$_POST['sort']}', '{$_POST['kind']}', '{$_POST['url']}', '{$_POST['target']}', '{$_POST['content']}')    
    "; //die($sql);
    $db->query($sql) or die($db->error() . $sql);
    $sn = $db->insert_id;
    $msg = "選單資料新增成功"; 
...
function op_form($kind,$sn=""){
  ...
  if($sn){
    $row = getKindsBySn($sn);    
    $row['pic'] = getFilesByKindColsnSort($kind,$sn);//圖片
    $row['op'] = "op_update";
  }else{
    $row['op'] = "op_insert";
  }
